  ...
  return <>
    ...
    <div className="grid4">
      {workspace.membership.role === "owner" ? <>
        <div className="card metric"><div className="metricLabel">Hotel revenue</div><div className="metricValue">{money(Number(calculation?.revenue ?? 0), workspace.organization.baseCurrency)}</div><div className="metricMeta">Finished performances only</div></div>
        ...
        <div className="card metric"><div className="metricLabel">Assigned shows</div><div className="metricValue">{showCount ?? 0}</div><div className="metricMeta">Available for scheduling</div></div>
    ...
    <section className="card section">
      ...
      <div className="sectionBody"><div className="list">
        {(upcoming ?? []).map(item => {
          ...
          return <div className="listItem" key={item.id}><div><div className="strong">{show?.name ?? "Untitled show"}</div><div className="sub">{date.toLocaleString("en-GB", { dateStyle: "medium", timeStyle: "short", timeZone: workspace.organization.timezone })} · {hotel?.name ?? "No hotel"}</div></div><span className="badge brand">Planned</span></div>;
        })}
