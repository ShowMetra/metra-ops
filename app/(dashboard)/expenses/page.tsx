...
export default async function ExpensesPage({ searchParams }: { searchParams: Promise<{ error?: string; message?: string }> }) {
  ...
  return <>
    <div className="pageHeader"><div><p className="eyebrow">Partner workspace</p><h1>Expenses</h1><p className="lede">Partners submit expenses for their shows. The owner reviews them before closing the month.</p></div></div>
    {error && <div className="notice danger">{error}</div>}{message && <div className="notice success">{message}</div>}
    <div className="grid2">
      <section className="card"><div className="sectionHeader"><h2>New expense</h2><span className="badge">Draft or submit</span></div><div className="sectionBody">
        {shows?.length ? <form action={createExpense} className="formGrid">
          ...
          <div className="field full buttonRow"><button name="intent" value="draft" className="button" type="submit">Save draft</button><button name="intent" value="submitted" className="button primary" type="submit">Submit to owner</button></div>
