...
export default async function PublicSchedulePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!live) return <div className="publicShell authShell"><main className="card authCard"><div className="brand"><span className="brandMark">R</span>Remarc Entertainment</div><p className="eyebrow">Artist schedule</p><h1>Link unavailable</h1><p className="lede">This schedule link is invalid, expired or has been disabled. Ask the partner for a new link.</p></main></div>;
  ...
  return <div className="publicShell">
    <header className="publicHeader"><div className="publicHeaderInner"><div className="brand" style={{ margin: "0 0 34px" }}><span className="brandMark">R</span>Remarc Entertainment</div><p className="eyebrow" style={{ color: "#c9c1ff" }}>Artist schedule</p><h1>{showName}</h1><p className="lede" style={{ color: "#cbc8d9" }}>Upcoming performances and schedule changes.</p></div></header>
