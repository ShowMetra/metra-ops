...
export default async function PublicSchedulePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const showName = live?.show.name ?? "Acrobatic Pirate";
  ...
  return <div className="publicShell">
    ...
    <main className="publicContent">
      <section className="card scheduleCard">{schedule.map(item => <div className="scheduleItem" key={`${item.date}-${item.show}-${item.time}`}><div className="dateBox"><strong>{item.date.split(" ")[1]}</strong><span>{item.day}</span></div><div><div className="strong">{item.hotel}</div><div className="sub">{item.time} · {item.show}</div></div><span className={`badge ${item.status.toLowerCase() === "completed" ? "success" : "brand"}`}>{item.status}</span></div>)}</section>
