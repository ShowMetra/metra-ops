...
export default async function PublicSchedulePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!live) return <div className="publicShell authShell"><main className="card authCard"><Brand /><p className="eyebrow">Artist schedule</p><h1>Link unavailable</h1><p className="lede">This schedule link is invalid, expired or has been disabled. Ask the partner for a new link.</p></main></div>;
  ...
  return <div className="publicShell">
    <header className="publicHeader"><div className="publicHeaderInner"><Brand className="publicBrand" /><p className="eyebrow" style={{ color: "#c9c1ff" }}>Artist schedule</p><h1>{showName}</h1><p className="lede" style={{ color: "#cbc8d9" }}>Upcoming performances and schedule changes.</p></div></header>
